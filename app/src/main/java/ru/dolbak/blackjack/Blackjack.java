...
class Deck {
    ...
    Rate[] rates = {Rate.TWO, Rate.THREE, Rate.FOUR, Rate.FIVE, Rate.SIX, Rate.SEVEN,
            Rate.EIGHT, Rate.NINE, Rate.TEN, Rate.JACK, Rate.QUEEN, Rate.KING, Rate.ACE};

    public Deck() {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 13; j++) {
                cards[i * 13 + j] = new Card(rates[i], suits[i]);
            }
        }
        ...
        for (int i = 0; i < cards.length; i++) {
            ...
            cards[randomIndex] = cards[cards.length - 1];
